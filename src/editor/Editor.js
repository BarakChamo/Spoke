...
export default class Editor extends EventEmitter {
  ...
  reparent(object, newParent, newBefore, useHistory = true, emitEvent = true, selectObject = true) {
    ...
    if (newParent !== object.parent) {
      let callback
      if (object.onReparent) {
        try {
          callback = object.onReparent(newParent)
        } catch (e) {
          this.emit('error', e)
          return
        }
  ...
  setProperty(object, propertyName, value, useHistory = true, emitEvent = true, disableCopy = false, target = null) {
    if (useHistory) {
    ...
    if (value && value.copy && !disableCopy) {
      if (target) object[target][propertyName].copy(value)
      else object[propertyName].copy(value)
    } else {
      if (target) object[target][propertyName] = value
      else object[propertyName] = value
    }
  ...
  setPropertyMultiple(objects, propertyName, value, useHistory = true, emitEvent = true, target = null) {
    if (useHistory) {
      return this.history.execute(new SetPropertyMultipleCommand(this, objects, propertyName, value, target))
    }

    for (let i = 0; i < objects.length; i++) {
      this.setProperty(objects[i], propertyName, value, false, false, false, target)
    }
  ...
  setPropertySelected(propertyName, value, useHistory = true, emitEvent = true, target = null) {
    return this.setPropertyMultiple(this.selected, propertyName, value, useHistory, emitEvent, target)
  }
